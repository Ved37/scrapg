...
function ProtectedPage({ children }) {
  ...
  const readNotifications=async()=>{ //why we dont add loader because when we read notifications and in background it will be cleared
    try {
      ...
      <div>
        <div className="flex justify-between items-center bg-primary p-5">
          ...
          <div className="bg-white flex gap-1 items-center py-2 px-5 rounded">
            ...
            <Badge count={notifications.filter((notification)=>!notification.read).length} onClick={()=>{readNotifications();setShowNotifications(true)}}
